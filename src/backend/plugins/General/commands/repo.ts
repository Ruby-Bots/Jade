    ...
    execute: async ({ ctx, client }) => {
        ...
        ctx.reply({
            embeds: [new Embed({
                ...
                description: `>>> ${commit.map((c) => c ? `[**${c.sha}**](${c.html_url}) - ${c.commit.author.date.slice(0, 10).replace("-", "/").replace("-", "/")} ([**${c.committer.login}**](${c.committer.html_url}))` : "No commit found!").join("\n")}`
        })]})
    }
})