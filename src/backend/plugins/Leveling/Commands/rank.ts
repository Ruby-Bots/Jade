...
export default new Command({
  ...
  execute: async ({ ctx, args }) => {
    ...
    if (!member) {
      return ctx.reply({ embeds: [new Embed({ description: `Could not find a user! Please try again later`})]})
    }

    const Level = await Levels.findOne({ userId: member.user.id, guildId: ctx.guild.id })
    if (!Level) {
      return ctx.reply({ embeds: [new Embed({ description: `Could not find any leveling data for this member!`})]})
    }

    ctx.reply({
      embeds: [
        new Embed({
          author: { name: `${member.user.tag}'s Level`, icon_url: member.user.displayAvatarURL({ dynamic: true }) },
